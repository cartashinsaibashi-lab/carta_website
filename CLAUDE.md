...
Google Drive の親フォルダを「リンクを知っている全員が閲覧可」で共有してもらい、
**`YYYY-MM-DD #番号 大会名`** というフォルダ名で大会と突き合わせる(`lib/drive.mjs`)。日付が必須なのは
...
**フォルダの段数は種別ごとに固定**(#114 / 2026-09-03 運営決定)。
**WOLF と宴は「種別 / シリーズ / 大会」の 3 段**(シリーズフォルダは `2026 #02` のような運営の表記)、
**歌留多は「種別 / 大会」の 2 段**。`lib/drive.mjs` の `SERIES_CATEGORIES` で決め打ちしてある —
形を見て自動で降りる作りにすると、置き場所を間違えても静かに動いてしまい
「写真が出ないまま気付かれない」事故に戻るため。決められた場所以外にある大会フォルダは
`misplaced` に入れて警告ログに出す(親フォルダ直下 / WOLF・宴 の種別フォルダ直下の 2 通り)。
**Player's Guide のフォルダはシリーズを挟む種別でも種別フォルダ直下のまま。**
**シリーズ名の表示は PokerLens の `behaviour.league.name`** で、フォルダ名は見出しに使わない
(種別が入らない短縮表記で、運営が手で付けるため表記ゆれる)。

**フォルダ名の先頭の番号は PokerLens と食い違う**(#82)。管理画面の No. は数値しか入らないため、
特殊イベントとサテライトの表記(`#SP2` / `#S1`)は `dailyDetails.shortName` にしかなく、
PokerLens が組み立てる `ev.name` は `#2 白豚 …` になる。一方で運営は管理表記のままフォルダを作るので
先頭が食い違い、完全一致にも包含にも掛からない(実際に 4 大会の写真が出ていなかった)。
`pickFolder()` は **完全一致 → 番号を外した名前の一致 → 包含** の順で照合する。
**大会側の「番号を外した名前」は正規表現で作らず `dailyDetails.name` を使う** — PokerLens が
番号を含まない大会名を別に持っており(934 件すべてに入っている)、正規表現だと
「XPT 3枠保証サテライト」のように番号と紛らわしい先頭語(25 件)を削り過ぎるため。
番号を外すと同日の別大会とぶつかりうるので、**候補が 1 つに絞れるときだけ**採用する。
どの段で当たったかは `/api/photos/:id` の `folder.match`(`exact` / `base` / `partial`)で分かる。
**フォルダ名に入れる番号は `#4` / `#SP2` / `#S1` の 3 種だけで、フライト表記
(`#3/A` や `(1A)`)は運営が管理しない**(2026-08-27 合意)。フライトの日は大会名自体に
「DAY 1A」が入っている(`dailyDetails.name` も `MAIN EVENT DAY 1A`)ので括弧が無くても
別日を区別できる — 逆に**大会名の「DAY 1A」を削ると出なくなる**。live の 62 フォルダで、
括弧あり・括弧なし・番号なしのどれでも 62 件全部が当たることを確認済み。
**日付だけはずれると救えない** — 照合は開催日で絞るため。実データで
`#5 PINEAPPLE POKER` のフォルダが 1 日ずれていて出ていなかった(2026-08-27 に運営がリネームして解消)。

**Results の優勝 / FT 写真は Drive のファイル名で選ぶ**(#80)。運営が既に
`mtNNN_Win.jpg` / `mtNNN_FT.jpg` / `mtNNN_HU.jpg` / `チップリ` と種類を書き分けていたので、
台帳もサブフォルダも作らずこれに乗っている。**表示するのは拡張子を除いた名前が `_Win` /
`_FT` で終わるもの**で、見せたい 1 枚だけその名前にしてもらう運用(他は `mt002_Win2` のように
末尾を変える)。`mtNNN` は種類ごとの連番で大会の識別子ではない — 同じ大会に FT が 11 枚、
Win が 4 枚入っていることがある。規約が守られていなくても壊さず、名前の自然順で先頭を出して
...
- **横スライド(横スクロール)を作らない**(#84)。最小幅は **iPhone 12 = 390px** で、
  そこで収まらないものは列幅の配分・折り返し・ラベルの短縮で収める。表に `min-width` を
  付けない(かつて `.data-table` にあった一律 460px / 400px は削除済み)。
  **例外は日付セレクター(`.date-strip-scroll`)だけ** — 1 か月分を 5.5 日ずつ見せる既存設計。
