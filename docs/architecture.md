...
Drive 親フォルダ(「リンクを知っている全員が閲覧可」で共有 = PHOTO_DRIVE_FOLDER_ID)
  └ 「WOLF」/「宴」/「歌留多」の種別フォルダ(#72)  ← この 1 段で写真の種別が決まる
       └ 「YYYY-MM-DD #番号 大会名」のサブフォルダ  ← この名前で大会と突き合わせる
            └ 写真
```
...
- **先頭の番号は運営の管理表記(`#4` / `#SP2` / `#S1`)で書いてよい**(#82)。管理画面の No. は
  数値しか入らず、`#SP2` は PokerLens 側の大会名(`#2 白豚 …`)に出てこないため、
  完全一致では拾えない。`pickFolder()` が **番号を外した名前**でも照合する
  (完全一致 → 番号除去一致 → 包含の順。どの段で当たったかは `/api/photos/:id` の `folder.match`)。
- **フライト表記(`#3/A` / `(1A)`)は運営が管理しない**(2026-08-27 合意)。Day 1A / Day 2 は
  大会名(`dailyDetails.name` = `MAIN EVENT DAY 1A`)で区別できるため。既存の括弧付きフォルダも
  そのまま当たる — live の 62 フォルダで、括弧あり・なし・番号なしのどれでも全件一致を確認済み。
